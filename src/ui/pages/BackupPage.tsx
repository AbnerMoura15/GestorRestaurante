  ...
  const handleExportCSV = () => {
    ...
    for (const product of products) {
      ...
      const channelsToShow: [SalesChannel, number][] = [
        ['PRESENCIAL', product.precoVendaPresencial], ['IFOOD_BASICO', product.precoVendaIfood],
        ['IFOOD_ENTREGA', product.precoVendaIfood], ['FOOD99', product.precoVenda99food],
      ]
      for (const [canal, preco] of channelsToShow) {
        ...
        rows.push([product.nome, channelLabels[canal], formatCurrencyBRL(preco), formatCurrencyBRL(custo),
          formatCurrencyBRL(result.fees.custoTotalTaxas), formatCurrencyBRL(result.lucroLiquido),
    ...
    <div className="p-4 space-y-4 max-w-lg mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3">
        ...
        <button onClick={handleExportJSON}
          className="w-full flex items-center gap-3 p-4 bg-brand-50 border border-brand-200 rounded-xl text-brand-800 text-sm hover:bg-brand-100">
        ...
        <button onClick={handleExportCSV}
          className="w-full flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl text-green-800 text-sm hover:bg-green-100">
          <FileText size={20} className="text-green-700" />
          <div className="text-left">
            <p className="font-semibold">Exportar CSV (margens por canal)</p>
            <p className="text-xs text-green-600 font-normal">Resumo de produtos, custos e margens</p>
          </div>
        </button>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3">
        <h3 className="text-sm font-semibold text-gray-700">Importar</h3>
        <label className="w-full flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-xl text-orange-800 text-sm cursor-pointer hover:bg-orange-100">
      ...
      <div className="text-xs text-gray-400 text-center">Dados salvos localmente neste dispositivo.</div>
    </div>
  )
}
