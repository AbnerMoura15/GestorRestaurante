...
export default function DashboardPage() {
  ...
  const getPreset = (canal: string): PlatformFeePreset | null =>
    platformMap.get(canal as SalesChannel) ?? null
          ...
          return (
            <div key={product.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="bg-brand-700 text-white px-4 py-3">
                ...
                <p className="text-brand-200 text-xs">{product.tamanhoMl}ml · Custo: {formatCurrencyBRL(custo)}</p>
              </div>
              <div className="divide-y divide-gray-100">
                {channelsToShow.map(canal => {
                  ...
                  const preco = canal === 'PRESENCIAL'
                    ...
                    : canal === 'FOOD99' ? product.precoVenda99food : product.precoVendaIfood
                  ...
                  return (
                    <div key={canal} className="px-4 py-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{formatCurrencyBRL(preco)}</p>
                      </div>
